...
export const drawPieChart = (
  selector: string,
  data: ChartData[],
  colors: readonly string[] = d3.schemeAccent,
) => {
  ...
  const svg = d3.select(svgNode).attr('width', size).attr('height', size);

  const group = svg
    .append('g')
    .attr('transform', `translate(${size / HALF_DIVIDER}, ${size / HALF_DIVIDER})`);
  ...
  const path = group
    ...
    .on('mousemove', (event: MouseEvent) => {
      tooltip.style.left = event.pageX + TOOLTIP_OFFSET.X + 'px';
      tooltip.style.top = event.pageY - TOOLTIP_OFFSET.Y + 'px';
    })
  ...
  path
    .style('opacity', 0)
    .transition()
    .delay((_: d3.PieArcDatum<ChartData>, index: number) => index * PIE_ANIMATION_DELAY)
