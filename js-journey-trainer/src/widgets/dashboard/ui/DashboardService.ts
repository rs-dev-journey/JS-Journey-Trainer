        ...
        const streakCard = createElement('div', {
            ...
            children: [
                ...
                createElement('div', {
                    ...
                    children: days.map(active => createElement('div', {
                        classList: ['dot', active ? 'active' : null].filter((className): className is string => !!className)
                    }))
                })
            ]
        });
    ...
    async init(): Promise<void> {
        ...
        chartConfigs.forEach(conf => {
            const card = createElement('div', {
                classList: ['chart-card', conf.wide ? 'wide' : 'not-wide'],
                children: [
                    createElement('h3', { textContent: conf.title }),
                    createElement('div', {
                        attributes: { id: conf.id },
            ...
            const chartContainer = document.getElementById(conf.id);
            if (chartContainer) {
        ...
        try {            
            ...
            const [quiz, asyncSorter, trueFalse] = await Promise.all([
                ...
                DataRepository.getActivity(1),
                DataRepository.getActivity(2)
            ]);
